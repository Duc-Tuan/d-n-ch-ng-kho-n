        ...
        */}
        <div className="min-h-dvh bg-canvas lg:grid lg:h-dvh lg:min-h-0 lg:grid-cols-[minmax(24rem,38%)_minmax(0,1fr)] lg:overflow-hidden">
          {/* ---------- Cột thương hiệu (chỉ desktop) ----------
              ...
              cho hai ô một hàng. */}
            {/* Nền loang thương hiệu: nền phẳng tuyệt đối ở cỡ này trông như một mảng chết, còn
              ba vệt chuyển màu rất nhạt cho cột chiều sâu mà không tranh chấp với chữ. Toàn bộ
              là gradient chứ không phải ảnh — không thêm một lượt tải nào, và tự đổi theo bảng
              màu vì màu lấy từ biến (xem `.mesh-brand` trong `globals.css`). */}
          ...
          <div className="relative flex min-h-dvh flex-col lg:h-full lg:min-h-0">
            {/* Trên desktop không có thanh trên nào để gắn nút đổi nền, nên nó nổi ở góc phải.
                Vẫn phải có mặt: người dùng quyết định nền sáng hay tối ngay ở màn đầu tiên chứ
                không đợi tới khi đăng nhập xong. */}
            <ThemeToggle className="absolute right-4 top-4 z-10 hidden lg:flex" />
            ...
            <header className="shrink-0 border-b border-line bg-surface lg:hidden">
              <div className="mx-auto flex h-14 max-w-md items-center px-4">
                ...
                {/* Người chưa đăng nhập cũng phải đổi được nền: màn này là màn đầu tiên họ thấy,
                    và với nhiều người là màn duy nhất trước khi quyết định có đăng ký hay không. */}
